...
* 在一个控制周期内( $f = 1000Hz$ )，电机的实际角速度 $\Omega_{cur}$ 相对于实际力矩的变化是很小的。、
...
$$
\tau_i = \frac{-\omega_i + \sqrt{\omega_i^2 - 4k_2(k_1\left|\omega_i\right| + k_3 / n - P_i)}}{2k_2}
$$
...
虽然使用线性关系已经足够，但我们可以采用一个过渡函数，以在剩余能量相对较高时扩展功率使用区间，而在剩余能量相对较低时迅速收缩功率使用。这种方式能够更灵活地管理功率输出。![1726999532508](images/1726999532508.png)
...
$$
P_{max} = P_{ref}^{max} - K_pe(t) - K_d\frac{e(t)-e(t-1)}{\Delta t}
$$
...
的情况下做到相应的错误处理
以下是出现不同情况错误处理的简图
![1726999022403](images/1726999022403.png)
